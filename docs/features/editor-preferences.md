...
**The loading skeleton is centered too.** While the page data loads, `CanvasTransformLayer` renders skeleton frames (`canvas-loading-frame-<id>`) instead of real ones. The same effect runs during this phase (when `canvasPage` is still null) and centers the skeleton for the `defaultBreakpoint` *preference* — i.e. what `activeBreakpointId` will resolve to once the site loads — so `centerOnBreakpointFrame` matches both the real wrapper and its loading skeleton. The result is that the skeleton and the real content share the same centered position, with no jump from a left-aligned skeleton to a centered canvas. There is no separate statically-positioned bootstrap skeleton in `AdminCanvasLayout` anymore (it was anchored at a fixed offset and caused exactly that jump); while the editor-body chunk loads, the canvas stage is simply empty until `CanvasRoot` mounts and shows the centered pannable skeleton.
