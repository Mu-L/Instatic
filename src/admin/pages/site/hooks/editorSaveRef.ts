 ...
 * read tools (`read_page_tree`, `read_styles`, content reads) hit the DB, so a
 * browser write that only landed in the editor store would be invisible until
 * the 30 s autosave fires. `usePersistence` registers its save callback here;
 * `useEditorMcpBridge` calls `flushEditorSave()` right after a mutating tool so
 * a follow-up headless read sees the change.
 */
